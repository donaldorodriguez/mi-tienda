...
const CONFIG = {
  ...
  // Datos de cobro
  NEQUI_NUMERO       : '300 384 3676',
  BANCOLOMBIA_CUENTA : '085-294163-60',
  LLAVE_BREB         : '3003843676',
  NOMBRE_TITULAR     : 'Donaldo',           // ← pon tu nombre completo como aparece en la cuenta
...
function promptVerificarCaptura() {
  return `Eres un verificador de pagos para una tienda de materiales ICFES en Colombia.

PAGO ESPERADO:
- Producto: ${PRODUCTO.nombre}
- Monto exacto: $${PRODUCTO.precio.toLocaleString('es-CO')} COP

CUENTAS AUTORIZADAS:
- Nequi: ${CONFIG.NEQUI_NUMERO} — titular: ${CONFIG.NOMBRE_TITULAR}
- Bancolombia: ${CONFIG.BANCOLOMBIA_CUENTA} — titular: ${CONFIG.NOMBRE_TITULAR}
- Bre-B / llave: ${CONFIG.LLAVE_BREB} — titular: ${CONFIG.NOMBRE_TITULAR}

VERIFICA EN LA IMAGEN:
1. ¿Es una captura real de una app bancaria colombiana? (Nequi, Bancolombia, Daviplata, etc.)
2. ¿El monto es exactamente $${PRODUCTO.precio.toLocaleString('es-CO')} COP?
3. ¿La fecha es de hoy o máximo ayer?
4. ¿Hay un número de transacción, referencia o código único visible?

RESPONDE ÚNICAMENTE con este JSON (sin texto adicional, sin bloques de código):
{
  "valido": true,
  "monto_detectado": 15000,
  "fecha_detectada": "texto de la fecha o null",
  "numero_transaccion": "código único o null",
  "banco_detectado": "nombre del banco o null",
  "nombre_pagador": "nombre visible o null",
  "razon_rechazo": null
}
...
async function procesarTexto(telefono, texto) {
  ...
  if (accion?.accion === 'mostrar_pago') {
    ...
    const instrucciones =
      `✅ ¡Perfecto! Para tu *${PRODUCTO.nombre}*\n\n` +
      `Valor: *$${PRODUCTO.precio.toLocaleString('es-CO')} COP*\n\n` +
      `Puedes pagar por:\n\n` +
      `*Nequi 📱*\n${CONFIG.NEQUI_NUMERO}\n\n` +
      `*Bancolombia 🏦*\n${CONFIG.BANCOLOMBIA_CUENTA}\n\n` +
      `*Bre-B 🔑*\n${CONFIG.LLAVE_BREB}\n\n` +
      `A nombre de: *${CONFIG.NOMBRE_TITULAR}*\n\n` +
